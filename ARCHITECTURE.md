...
* This chapter defines the entry point of our zero-trust sidecar. The proxy acts as a local chokepoint for all outbound traffic from the protected application, utilizing Python's asyncio to handle high concurrency without blocking.

* The Routing Mechanism: How the proxy captures outbound HTTP/TCP requests (e.g., configuring the local environment to route via localhost:PORT).

* Connection Handling: Utilizing asynchronous streams to accept incoming local connections and prepare the outgoing connection to the target server.

* Payload Extraction: Reading the request headers and body into memory. We will discuss the limits on payload size to prevent memory exhaustion (OOM) and latency spikes.

* The Trade-off: Why Python is chosen for the I/O bound routing (rapid development, excellent async ecosystem) despite its limitations with CPU-bound tasks.

Chapter 2: The Bridge (The Python-Rust FFI Boundary)
Moving data between two different runtime environments introduces overhead. This chapter details the Foreign Function Interface (FFI) using PyO3, which is the most critical performance bottleneck in a polyglot architecture.

* Data Serialization Strategy: How we represent the network payload. We will explore passing raw byte arrays (&[u8]) to avoid the costly overhead of serializing/deserializing JSON or heavy Python objects.

* Memory Management: Defining who owns the memory. We must ensure that the Python garbage collector does not clean up the payload while Rust is scanning it, while also avoiding unnecessary memory copying (Zero-Copy architecture where possible).

* Bypassing the GIL: Documenting how PyO3 allows the Rust engine to release Python's Global Interpreter Lock (GIL) during the scanning phase, ensuring that our heavy CPU-bound heuristics do not block other asynchronous routing tasks.

Chapter 3: The Engine (High-Speed Heuristic Evaluation)
Once the payload crosses the FFI boundary, the Rust engine takes over. This chapter covers the internal mechanics of the high-performance Data Exfiltration Prevention engine.

* The Scanning Heuristics: The primary methods of detection, combining fast multi-pattern string searching (like the Aho-Corasick algorithm) for known secret signatures (e.g., AKIA... for AWS) with Shannon Entropy calculations to detect highly randomized, base64-encoded strings that often represent private keys.

* Failing Fast: The architectural decision to prioritize speed. The engine must short-circuit and return a Drop signal the absolute millisecond a critical secret is matched, rather than scanning the remainder of the payload.

* Thread Safety & State: Ensuring the Rust engine remains stateless and thread-safe so it can be called concurrently by multiple asynchronous Python workers without race conditions.

Chapter 4: The Verdict (Asynchronous Enforcement & Auditing)
The final phase occurs when the Rust engine passes a boolean or enum (Allow or Drop) back to the Python proxy. This chapter details the enforcement of the zero-trust policy.

* Connection Termination: If a Drop signal is received, how the proxy aggressively terminates the connection. For HTTP traffic, returning an immediate 403 Forbidden local response; for raw TCP, issuing a TCP RST (Reset) packet.

* Connection Passthrough: If an Allow signal is received, how the proxy efficiently pipes the buffered payload to the external target server and streams the response back to the local application.

* The Audit Trail: How enforcement events are logged. We will detail the secure logging mechanism that records the destination and timestamp of blocked exfiltration attempts without ever logging the plaintext secret itself.
...
Chapter 3: The Engine (High-Speed Heuristic Evaluation)Once the network payload crosses the FFI boundary into Rust, the core Data Exfiltration Prevention engine takes over. This module is strictly CPU-bound. Its architectural mandate is to scan raw byte streams for sensitive data with near-zero latency, ensuring the proxy does not bottleneck outbound network traffic.3.1 The Multi-Pattern Matrix (Aho-Corasick Algorithm)Standard Regular Expressions (Regex) are incredibly inefficient for scanning large payloads against hundreds of different secret signatures (e.g., AWS keys, GitHub tokens, Stripe API keys). A naive regex approach evaluates patterns sequentially, leading to severe performance degradation as the signature database grows.The Solution: Aegis utilizes the Aho-Corasick algorithm, a finite-state machine that searches for multiple string patterns simultaneously.Time Complexity: Aho-Corasick guarantees an $O(n + m + z)$ time complexity, where $n$ is the length of the payload chunk, $m$ is the total length of all patterns, and $z$ is the number of matches. This means the evaluation time remains largely constant whether we are scanning for 10 secret signatures or 10,000. It reads the byte stream in a single pass.3.2 Entropy & Heuristic AnalysisWhile Aho-Corasick efficiently catches known, structured secrets (e.g., strings starting with AKIA... or ghp_...), it cannot catch undocumented or custom cryptographic keys. To detect these, the Rust engine employs heuristic analysis.Shannon Entropy Calculation: The engine identifies highly randomized, unstructured strings (such as Base64-encoded RSA private keys) by calculating their information density using Shannon Entropy:$$H = -\sum_{i=1}^{n} p_i \log_2(p_i)$$Where $p_i$ is the probability of a given character appearing in the evaluated chunk. If a string block exceeds a specific entropy threshold (e.g., $H > 4.5$ for alphanumeric strings), it is flagged as a potential cryptographic secret.Contextual Validation: Because high entropy alone can yield false positives (e.g., compressed image data or compiled binaries), the engine cross-references high-entropy blocks with surrounding contextual byte markers (e.g., Authorization: Bearer, private_key, or = padding in Base64) to finalize the verdict.3.3 The Fail-Fast Execution ModelIn a zero-trust sidecar, absolute security takes precedence over complete telemetry. The engine is not designed to catalog every single secret in a compromised payload; it is designed to stop the exfiltration immediately.Short-Circuiting: The Rust engine employs a strict fail-fast execution model. The absolute microsecond the Aho-Corasick automaton registers a critical match, or the entropy threshold alongside a context marker is breached, the evaluation loop short-circuits.Immediate Handoff: The engine immediately releases any remaining payload bytes, re-acquires the Python GIL, and passes a Drop signal back across the PyO3 boundary. This prevents wasting CPU cycles on an inherently compromised connection.3.4 Concurrency & Thread-Safe StateBecause the Python proxy routes traffic asynchronously, the Rust engine will be invoked concurrently by hundreds of different asyncio tasks.Stateless Scanning with Stateful Buffers: The core Aho-Corasick automaton and heuristic configurations are initialized once at startup and shared across threads as immutable, read-only references.Connection Context: To support the rolling buffer (defined in Chapter 2) needed to catch secrets spanning chunk boundaries, the Rust engine maintains an isolated, thread-safe state map (e.g., utilizing DashMap or a similar concurrent data structure). Each buffer is keyed to a unique connection ID passed from Python, ensuring that concurrent evaluations never corrupt each other's memory space.

This is the final piece of the puzzle. Chapter 4 is where the zero-trust policy is actually enforced. From an engineering perspective, catching the secret is only half the battle; how you handle the fallout without crashing the proxy or leaking the secret into your own system logs is what truly defines a production-ready DevSecOps tool.

When hiring managers are evaluating a repository to see if an engineer is ready for a high-level remote security role, they look closely at the telemetry and auditing. They need to know you understand compliance constraints.

Here is the final chapter for your ARCHITECTURE.md.
